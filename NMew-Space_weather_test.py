...
def _nz_risk_phrase(kind: str, level: str) -> str:
    # kind in {"R","S","G"}; level in {"ok","caution","watch","severe"}
    if kind == "R":  # Radio blackouts
        return {
...
def _class_to_level(cls_key: str) -> str:
    # Map UI class tags to a human term
...
def rewrite_to_nz(section: str, text: str, *,
                  ...
                  day1=None) -> str:
    """
    Convert NOAA/BOM technical summary into plain-English with NZ flavour.
    Inputs:
      - section: one of {"solar_activity","solar_wind","geospace","energetic_particle"}
      - text: raw NOAA/BOM text
      - r_now/s_now/g_now: current R/S/G labels (e.g., "R1","S0","G2")
      - day1: optional dict from 3-day parser for extra hints
    """
    tx = (text or "").strip()
    if not tx:
        base = "No significant activity reported."
    else:
        # Lightweight templates based on content cues
        low = tx.lower()
        ...
        else:  # energetic_particle or fallback
    ...
    # Add current/severity context in NZ terms
    # Classify with your existing helpers
    ...
    # If day1 forecast hints at higher G, add a forward-looking note
    g_hint = ""
    try:
...
with st.container():
    nz_sa = rewrite_to_nz("solar_activity", sa_full,
                          r_now=current['r'], s_now=current['s'], g_now=current['g'],
                          day1=day1)
    nz_sw = rewrite_to_nz("solar_wind", sw_full,
                          r_now=current['r'], s_now=current['s'], g_now=current['g'],
                          day1=day1)
    nz_gs = rewrite_to_nz("geospace", gs_full,
                          r_now=current['r'], s_now=current['s'], g_now=current['g'],
                          day1=day1)
    ...
    # --- Day 1–3 (uses narrative fallback flags) ---
    r1, r1_cls = r_label_and_class_for_day(day1, narrative_flags)
    s1, s1_cls = s_label_and_class_for_day(day1, narrative_flags)
    ...
    r2, r2_cls = r_label_and_class_for_day(day2, narrative_flags)
    s2, s2_cls = s_label_and_class_for_day(day2, narrative_flags)
    ...
    r3, r3_cls = r_label_and_class_for_day(day3, narrative_flags)
    s3, s3_cls = s_label_and_class_for_day(day3, narrative_flags)
...
with tab_forecast:
    ...
    def day_card(col, title, d):
        with col:
            st.markdown(f"### {title}")
            # Use the same narrative fallback flags here
...
fc_sa = (structured_disc.get("solar_activity", {}) or {}).get("forecast", "")
fc_sw = (structured_disc.get("solar_wind", {}) or {}).get("forecast", "")
fc_gs = (structured_disc.get("geospace", {}) or {}).get("forecast", "")
