...
UA = {"User-Agent": "SpaceWeatherDashboard/2.2 (+streamlit)"}

# ---------- Optional BOM wrapper ----------
HAVE_BOM = True
BOM_ERR = ""
try:
...
BOM_API_KEY = "BOM Key"
bom = SpaceWeather(BOM_API_KEY) if HAVE_BOM else None
...
# ==========================
def get_bom_aurora():
    if not HAVE_BOM or bom is None:
        return f"Aurora info unavailable. ({BOM_ERR or 'pyspaceweather not installed'})"
    try:
...
def pdf_download_button(noaa_discussion_raw, past, current, next24, bom_aurora, summary_text):
    ...
    try: pdf_output = pdf.output(dest='S').encode('latin1')
    except Exception: pdf_output = pdf.output(dest='S').encode('latin1', errors='ignore')
