...
if __name__ == "__main__":
    ...
    run()  # will be defined in step 2 below
    
def run():
    # MOVE all Streamlit UI code below into this function:
    # - the sidebar controls (st.sidebar…)
    # - the CSS styles (st.markdown(..., unsafe_allow_html=True))
    # - the tabs and all their contents
    # Keep your helper functions where they are (they can remain module-level).
    pass
...
# ---------- config ----------
UA = {"User-Agent": "SpaceWeatherDashboard/4.0 (+streamlit)"}
DATA_DIR = st.secrets.get("DATA_DIR", r"C:\Users\Standalone1\Desktop\Operation\data")
NOAA_API_KEY = st.secrets.get("NOAA_API_KEY", "")
BOM_API_KEY  = st.secrets.get("BOM_API_KEY", "51585962-2fdd-4cf5-9d9e-74cdd09e3bab")  # fallback
...
def get_bom_aurora():
    ...
    try:
        outlooks = bom.get_aurora_outlook(api_key=BOM_API_KEY)
        watches  = bom.get_aurora_watch(api_key=BOM_API_KEY)
        alerts   = bom.get_aurora_alert(api_key=BOM_API_KEY)
        lines = []
        if outlooks: lines.append(f"Outlook: {getattr(outlooks[0], 'comments', '(no text)')}")
        if watches:  lines.append(f"Watch: {getattr(watches[0], 'comments', '(no text)')}")
        if alerts:   lines.append(f"Alert: {getattr(alerts[0], 'description', '(no text)')}")
